...
def simp(r: Rexp) : Rexp = r match{
 case SEQ(r1, r2) => (simp(r1), simp(r2)) match {
    ...
    case (rg, ONE) =>  simp(rg)
    case (ONE, rg) =>  simp(rg)
    case (rg1, rg2) => SEQ(rg1, rg2)
    }
  case ALT(r1, r2) => (simp(r1), simp(r2)) match {
    case (rg, ZERO) => simp(rg)
    case (ZERO, rg) =>  simp(rg)
    case (rg1, rg2) => if(rg1 == rg2) simp(rg1) else ALT(rg1,rg2)
    }
  case _ => r
}
...
def matcher(r: Rexp, s: String): Boolean = {
  if (s.toList == Nil) nullable(r)
  else{
  nullable(simp(ders(s.toList, r)))  
  }
}
